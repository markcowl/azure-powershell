...
{
    [Cmdlet(VerbsCommon.New, "AzureRmDataLakeStoreItem", SupportsShouldProcess = true), OutputType(typeof (string))]
    public class NewAzureDataLakeStoreItem : DataLakeStoreFileSystemCmdletBase
    {
        ...
        public override void ExecuteCmdlet()
        {
            ...
            ConfirmAction(
                Force.IsPresent,
                string.Format("Are you sure you want to overwrite item {0}?", Path.TransformedPath),
                "Creating new item",
                Path.OriginalPath,
                () =>
                {
                ...
                () => DataLakeStoreFileSystemClient.TestFileOrFolderExistence(Path.TransformedPath, 
                    Account, out ignored));
        }
    }
}